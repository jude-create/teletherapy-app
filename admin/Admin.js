...
const TherapistReviewCard = ({ therapist, updating, onUpdate }) => {
...
const Admin = () => {
  ...
  const pendingTherapists = useMemo(
    () =>
      (dashboard?.therapists || []).filter(
        (therapist) => (therapist.verificationStatus || 'pending') === 'pending'
      ),
    [dashboard]
  );
  ...
  const handleVerificationUpdate = async (therapist, status) => {
    try {
