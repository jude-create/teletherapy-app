...
import type { Appointment as AppointmentModel } from '../types/models';

type PatientAppointment = AppointmentModel & {
  id: string;
  sessionLink?: string;
  therapist?: { name?: string; availability?: { slots?: unknown[] } };
  therapistName?: string;
};

const statusCopy: Record<string, { label: string; message: string; color: string; backgroundColor: string }> = {
  pending: {
    label: 'Pending',
...
const AppointmentCard = ({ appointment, onCancel, onMessage, onJoinSession, onReschedule, updating }: any) => {
...
export default function Appointment() {
  ...
  const loadAppointments = async () => {
    try {
      ...
      const nextAppointments = (await getAppointmentsForPatient(currentUser.uid)) as PatientAppointment[];
  ...
  const rescheduleAppointment = async (appointment: PatientAppointment) => {
