...
const getAvailableDays = (selectedDays = {}) =>
  Object.entries(selectedDays)
    .filter(([, selected]) => selected)
    .map(([day]) => day);

const getOverlap = (first = [], second = []) => {
...
const scoreTherapistMatch = (therapist, preferences) => {
...
const TherapistCard = ({ therapist, onViewProfile, onBook }) => {
...
const TherapistDetailModal = ({ therapist, visible, onClose, onBook }) => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        ...
        const therapists = await findTherapistsByTherapyType(preferences.selectedOption);

        setMatchedTherapists(
          therapists
            .map((therapist) => scoreTherapistMatch(therapist, preferences))
