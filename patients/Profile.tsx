...
const ChipList = ({ items, styles }: any) => {
  if (!items?.length) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled) {
        ...
        setUserData((current: any) => ({ ...current, profileImage: imageUrl }));
      }
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'Unable to update profile image.');
    }
  };
