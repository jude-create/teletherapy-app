...
const LogInScreen = () => {
  ...
  const updateField = (field: keyof LoginErrors, value: string) => {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if ((error as RoleSelectionError)?.needsRoleSelection) {
        navigation.navigate('Register');
        return;
      }
      setErrors({ form: getFriendlyAuthError(error as RoleSelectionError) });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleGooglePress = async () => {
    ...
    try {
      setSubmitting(true);
      setErrors({});
      idToken = await signInWithGoogleAsync();
      await signInExistingGoogleAccount(idToken);
    } catch (error) {
      if ((error as RoleSelectionError)?.needsRoleSelection && idToken) {
