import type { AvailabilitySlot, UpcomingSlotOption } from '../types/models';
...
type TherapistAvailability = Record<string, unknown> & {
...
export const formatSlotLabel = (slot?: Partial<AvailabilitySlot> | null) => {
  if (!slot) return 'Slot unavailable';
  return `${slot.day} ${slot.startTime} - ${slot.endTime}`;
};

export const getSlotsFromTherapist = (therapist: TherapistAvailability | null = {}): AvailabilitySlot[] => {
  if (!therapist) return [];
  ...
  const labeledSlots = String(therapist.time)
    ...
    .filter((slot): slot is AvailabilitySlot => Boolean(slot?.startTime && slot?.endTime));
...
export const getUpcomingSlotOptions = (
  slots: AvailabilitySlot[] = [],
  weeksAhead = 4
): UpcomingSlotOption[] => {
  const today = new Date();
  const options: UpcomingSlotOption[] = [];
