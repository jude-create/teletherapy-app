...
type UserLookup = string | Pick<User, 'uid' | 'email'> | null | undefined;

const withTimeout = <T>(promise: Promise<T>, ms = PROFILE_TIMEOUT_MS): Promise<T> =>
  Promise.race([
    promise,
...
export const isOfflineFirestoreError = (error: FirestoreError | unknown) => {
  const nextError = error as FirestoreError;
  return (
    nextError?.code === 'unavailable' ||
    /client is offline|offline|network|timed out/i.test(nextError?.message || '')
  );
};

const getProfileByEmail = async (email?: string | null): Promise<UserProfile | null> => {
...
export const getUserProfile = async (userOrUid: UserLookup): Promise<UserProfile | null> => {
...
export const getUserRoleWithRetry = async (userOrUid: UserLookup, attempts?: number, delayMs?: number) => {
