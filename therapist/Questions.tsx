...
type TherapistQuestionKey = 'therapyTypes' | 'specialties' | 'preferredPatientGroups' | 'availability';
type TherapistAnswers = {
  therapyTypes: string[];
  specialties: string[];
...
const OptionCard = ({ label, selected, onPress }: any) => (
  <Pressable
...
const Questions = () => {
  ...
  const [answers, setAnswers] = useState<TherapistAnswers>({
    therapyTypes: [],
    specialties: [],
  ...
  const toggleOption = (option: string) => {
    setErrorMessage('');
    setAnswers((previous) => {
      const current = previous[currentStep.key as keyof Pick<TherapistAnswers, 'therapyTypes' | 'specialties' | 'preferredPatientGroups'>] as string[];
      return {
        ...previous,
  ...
  const validateStep = (step = currentStep) => {
    ...
    if (step.key !== 'availability' && !(answers[step.key] as string[])?.length) {
          ...
          <View style={styles.options}>
            {currentStep.options.map((option) => (
              <OptionCard
                key={option}
                label={option}
                selected={((answers[currentStep.key as keyof Pick<TherapistAnswers, 'therapyTypes' | 'specialties' | 'preferredPatientGroups'>] || []) as string[]).includes(option)}
